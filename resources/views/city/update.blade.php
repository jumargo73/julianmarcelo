...
@section('subcontent')
    <div class="intro-y mt-8 flex items-center">
        <h2 class="mr-auto text-lg font-medium">Actualizar Ciudad</h2>
    </div>
                    ...
                    <div class="grid-cols-2 gap-2 sm:grid">
                        <!-- Codigo Dane -->                    
                            <x-base.form-input
                                ...
                                value="{{ old('code_dane',$city->code_dane) }}"
                            />
                            @error('name')
                ...
                <x-base.form-label for="name">Provincia</x-base.form-label>
                    <div class="grid-cols-2 gap-2 sm:grid">
                        <!-- Codigo Dane -->                    
                            <x-base.form-input
                                ...
                                value="{{ old('provincia',$city->provincia) }}"
                            />
                            @error('provincia')
                                <div class="text-red-500 text-sm mt-1">{{ $message }}</div>
                            @enderror 
                    ...
                    >
                        ...
                        @foreach ($departments as $department)
                            <option value="{{$department->id}}" {{ old('department_id',$city->id) == $department->id ? 'selected' : '' }}>{{ $department->code_dane }} - {{ $department->name }}</option>
                        @endforeach
                    </x-base.tom-select>
                    @error('department_id')
                        <div class="text-red-500 text-sm mt-1">{{ $message }}</div>
